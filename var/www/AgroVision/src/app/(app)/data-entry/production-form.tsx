...
export function ProductionForm() {
  ...
  const saveHarvestData = (values: ProductionFormValues) => {
    ...
    // The actual database operations are now wrapped in startTransition
  ...
  const onSubmit = (values: ProductionFormValues) => {
    // We wrap this entire logic in a transition so the UI doesn't lock up on AI call
    startTransition(async () => {
      const farmer = collectors.find(c => c.id === values.farmerId);
      if(!farmer) return;
      ...
      try {
          ...
          if (validationResult.isValid) {
           ...
           saveHarvestData(values); // Save directly if AI fails
      }
    });
  }
